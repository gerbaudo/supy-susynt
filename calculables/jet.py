...
#___________________________________________________________
class trueJets(supy.wrappedChain.calculable) :
    def __init__(self):
        maxEta, minPt = 2.5, 20.0
        self.maxEta = maxEta
        self.minPt  = minPt
        self.moreName = ""
        if maxEta : self.moreName += "|eta|<%.1f"%maxEta
        if minPt  : self.moreName += "pt>%.1f"%minPt
    def update(self, _) :
        self.value = [j for j in self.source['truthJets']
                      if abs(j.Eta())<self.maxEta and j.Pt()>self.minPt]
#___________________________________________________________

class indicesTrueJets(supy.wrappedChain.calculable) :
    def update(self, _) :
        self.value = [i for i,jj in enumerate(self.source['trueJets'])]
#___________________________________________________________
